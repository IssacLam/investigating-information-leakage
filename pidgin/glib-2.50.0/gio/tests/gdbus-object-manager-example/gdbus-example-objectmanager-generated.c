...
static const _ExtendedGDBusPropertyInfo * const _example_animal_property_info_pointers[] =
{
  ...
  &_example_animal_property_info_foo,
  &_example_animal_property_info_bar,
  NULL
};
  ...
  if (!_g_value_equal (value, &skeleton->priv->properties[prop_id - 1]))
    {
      if (g_dbus_interface_skeleton_get_connection (G_DBUS_INTERFACE_SKELETON (skeleton)) != NULL)
        _example_animal_schedule_emit_changed (skeleton, _example_animal_property_info_pointers[prop_id - 1], prop_id, &skeleton->priv->properties[prop_id - 1]);
